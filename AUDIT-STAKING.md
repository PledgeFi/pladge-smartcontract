...
7. ~~Bring [deployments/4663.json](deployments/4663.json) back in sync with the chain: three pools, 0 and 1 retired, pool 2 on `0x1BE30101…` (L-7). Settle on one name for that token, since the repo says "PONS" and the token says `PLG`.~~ Done: the file lists all three pools, and the repo now calls `0x1BE30101…` `PLG` everywhere. Still outstanding on chain: run [13_RenameStakingPools.s.sol](script/mainnet/13_RenameStakingPools.s.sol) so pool 2 reads `PLG Staking` and pools 0 and 1 are marked retired. Renaming is the only remedy available — `_pools` is append-only and a pool's tokens are fixed at `addPool`, so the two dead pools can never be deleted or re-pointed at the live PLG.
