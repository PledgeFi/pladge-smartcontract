# Pledge Finance: Full Project & Smart Contract Audit Briefing
...
The credit narrative was locked into `docs/product.md` during this phase — perpetual, receipt-style repayment, HF-only liquidation. Governance and PSM exist **only as preview UI**; there is no Governor and no PSM contract. The bridge is an EIP-712 attestation flow on the RH testnet, **not** a production bridge.
...
That mistake produced the standing rule: **every protocol deploy must be an ERC1967 proxy**. All protocol contracts were converted to UUPS via `PledgeUupsOwnable`.
...
| `PledgeReferralClaim` | `0x6D33001a3c4c548a32a32a3ADC675971fC1EFFa3` | n/a (direct deploy 2026-09-20, not a proxy) |
