...
contract StakingBudgetForkTest is Test {
    ...
    function test_budgetCoversCampaignExactly() public {
        ...
        _configurePool();
        _report("--- campaign setup ---");
        emit log_named_uint("campaign length (seconds)", campaign);
        emit log_named_decimal_uint("budget (PLG)", budget, 18);
        emit log_named_string("satuan waktu", unitName);
        emit log_named_decimal_uint("paid per unit (PLG)", rate * unitSeconds, 18);
        emit log_named_uint("rewardRatePerSecond (wei)", rate);
        emit log_named_uint("shortest lock (seconds)", minLock);
        emit log_named_uint("longest lock (seconds)", maxLock);
        emit log_named_uint("longest-lock multiplier (bps)", STAKING.multiplierBps(POOL, maxLock));
        ...
        vm.prank(budi);
        STAKING.claim(POOL);
        assertEq(PLG.balanceOf(budi), budiMid, "mid-campaign claim does not match pendingReward");
        ...
        _report("--- hasil ---");
        emit log_named_decimal_uint("Budi, longest lock (PLG)", budiEarned, 18);
        emit log_named_decimal_uint("Ani, shortest lock (PLG)", aniEarned, 18);
        emit log_named_decimal_uint("total paid             (PLG)", paid, 18);
        emit log_named_decimal_uint("reserve left           (PLG)", reserveLeft, 18);

        // The whole claim of this harness: the pool pays out its budget and stops, never more.
        assertLe(paid, budget, "paid more than the budget");
        ...
        assertLe(reserveLeft, campaign, "reserve not fully spent - emissions stalled");
        ...
        assertApproxEqRel(paid, budget, 0.000001e18, "too much stranded in rounding");
        ...
        if (stakeLong == stakeShort) {
            ...
            assertApproxEqRel(budiEarned * BPS, aniEarned * boost, 0.001e18, "boost split is off");
        }
    }
